...
public class WxCartController {
     ...
     */
    @PostMapping("add")
    @ApiOperation("加入商品到购物车,参数：{ goodsId: xxx, productId: xxx, number: xxx }")
     ...
     */
    @PostMapping("fastadd")
    @ApiOperation("立即购买，参数{ goodsId: xxx, productId: xxx, number: xxx }")
     ...
     */
    @PostMapping("update")
    @ApiOperation("修改购物车商品货品数量,参数{ id: xxx, goodsId: xxx, productId: xxx, number: xxx }")
     ...
     */
    @PostMapping("checked")
    @ApiOperation("购物车商品货品勾选状态(如果原来没有勾选，则设置勾选状态；如果商品已经勾选，则设置非勾选状态。)")
    public Object checked(@LoginUser Integer userId,
                          @ApiParam("购物车商品信息， { productIds: xxx, isChecked: 1/0 }") @RequestBody String body) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (body == null) {
     ...
     */
    @PostMapping("delete")
    @ApiOperation("购物车商品删除,参数{ productIds: xxx }")
    ...
    @ApiOperation("购物车商品货品数量,如果用户没有登录，则返回空数据")
