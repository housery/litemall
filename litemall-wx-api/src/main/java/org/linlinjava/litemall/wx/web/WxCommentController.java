...
public class WxCommentController {
     ...
     */
    @PostMapping("post")
    @ApiOperation("发表评论,参数：content,star,type,valueId,hasPicture")
     ...
     */
    @GetMapping("count")
    @ApiOperation("评论数量,type(类型ID。 如果是0，则查询商品评论；如果是1，则查询专题评论),valueId(商品或专题ID。如果type是0，则是商品ID；如果type是1，则是专题ID)")
     ...
     */
    @GetMapping("list")
    @ApiOperation("评论列表,参数:type(类型ID。 如果是0，则查询商品评论；如果是1，则查询专题评论)," +
            "valueId(商品或专题ID。如果type是0，则是商品ID；如果type是1，则是专题ID。)," +
            "showType(显示类型。如果是0，则查询全部；如果是1，则查询有图片的评论。)")
