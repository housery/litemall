...
public class WxOrderController {
     ...
     */
    @GetMapping("list")
    @ApiOperation("订单列表,参数：showType( 0，全部订单；1，待付款；2，待发货；3，待收货；4，待评价。)")
     ...
     */
    @GetMapping("detail")
    @ApiOperation("订单详情,参数：orderId（订单ID）")
     ...
     */
    @PostMapping("submit")
    @ApiOperation("提交订单,参数：{ cartId：xxx, addressId: xxx, couponId: xxx, message: xxx, grouponRulesId: xxx,  grouponLinkId: xxx}")
     ...
     */
    @PostMapping("refund")
    @ApiOperation("订单申请退款,参数：orderId")
     ...
     */
    @GetMapping("goods")
    @ApiOperation("待评价订单商品信息,参数：orderId（订单ID），goodsId（商品ID）")
     ...
     */
    @PostMapping("comment")
    @ApiOperation("评价订单商品,{ orderId：xxx }")
